<!-- 页面用途：聚合展示已发布 Honor，不生成 Honor 详情路由。 -->
...
const { data: response, error } = await useAsyncData(
  computed(() => `trust:honors:${locale.value}`),
  () => api<Envelope<PublicTrustListItemDto[]>>(`/public/trust/honors/${locale.value}`),
)
const { data: metadataResponse, error: metadataError } = await useAsyncData(
  computed(() => `page-metadata:honors:${locale.value}`),
  () => api<Envelope<PublicPageMetadataDto>>(`/public/page-metadata/honors/${locale.value}`),
)
if (error.value || metadataError.value || !metadataResponse.value?.data)
  throw createError({ statusCode: 500, statusMessage: 'Honors unavailable' })
...
<template>
  <div class="trust-aggregate">
    ...
    <header class="public-container trust-aggregate__header">
      ...
      <h1>{{ labels.trust.honorsTitle }}</h1>
      <p>{{ labels.trust.honorsIntro }}</p>
    </header>
